...
3. 看 Terminal B 印出 `unsave.json` 候選，輸入 `y` 或 `n` 確認是否執行取消儲存。
...
9. 勾選 **自動載入 unsave.json**。若要使用 Terminal B 的確認 gate，**載入後自動取消儲存** 請不要勾；`agent_driver.py scrape` 會在你輸入 `y` 後執行一次性取消儲存。
10. 用下面指令確認：
...
若要回到舊版「完全自動取消儲存」流程，加上 `--no-unsave-confirm`。`--unsave-timeout-seconds`（預設 `600`）控制 gate 等待新 `unsave.json` 的最長秒數。
