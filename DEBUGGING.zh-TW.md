...
| `RuntimeError: ANTHROPIC_API_KEY is required for the Anthropic backend` | `LLM_PROVIDER=anthropic` 但 `.env` 只填了 `GEMINI_API_KEY` | 在 `.env` 補上 `ANTHROPIC_API_KEY=...`，或把 `LLM_PROVIDER` 改回 `gemini` |
| `RuntimeError: OPENAI_API_KEY is required for the OpenAI backend` | OpenAI backend 同樣狀況 | 補 `OPENAI_API_KEY=...` 或改 provider |
| `ValueError: unsupported llm provider: 'foo'` | `LLM_PROVIDER` 或 `llm.provider` 拼錯 | 用 `gemini` / `anthropic` / `openai` 三選一 |
| 設了 model override 卻沒套用 | 優先順序：`THREADS_LLM_*_MODEL` 環境變數 > 舊版 env（`CLASSIFIER_MODEL` / `THREADS_GEMINI_TITLE_MODEL` / `IMAGE_OCR_MODEL`）> `config.json` 的 `llm.text-model` / `title-model` / `vision-model` > 各 provider 預設 | 確認你是在哪個層級設的；如果上層被舊變數蓋過，先清掉那一層 |
