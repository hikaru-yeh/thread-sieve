...
| `probe` 報 `scriptVersion=... expected 0.3.0` | `SCRIPT_VERSION` constant 跟 `@version` header 對不上 | 在 `userscripts/threads-scriber-auto.user.js` 把兩處都 bump；或重跑 `scripts/_rebuild_userscript.py` |
